...
end entity FOC_core_fuzzy;

architecture behavioral of FOC_core_fuzzy is

component FOC_fuzzyPID is
    port (
        en          : in  std_logic;
        n_res       : in  std_logic;
        CLK         : in  std_logic;
    ...
    signal positional_reading                                  : sfixed (0 downto -17);
    signal positional_reading_currentSetpoint                                  : sfixed (0 downto -17);
    signal electricBrake_sfixed                                : sfixed (0 downto -17);
    signal electricBrake                                       : std_logic;
    constant positional_setpoint                               : sfixed (0 downto -17) := to_sfixed(0.0, 0, -17);
...
begin
    ...
    torqueVectorGenerator_3levelSwitching : component FOC_3levelSwitching
        port map(
            ...
            reading      => positional_reading,
            setpoint     => positional_setpoint,
            output       => vectorPosition
            );

--    PID_position : component PID
--        generic map (
...
--            min_pid_pid => to_sfixed(-1.0, 0, -17),
--            pid_out     => vectorPosition
--            );
    ...
    PID_currentSetpoint : component FOC_fuzzyPID
--        generic map (
        --sampling_time => 0.000000064,  --64ns
--            )
        port map (
            ...
            n_res       => n_res,
            CLK         => CLK,
            kp          => kp_currentSetpoint,
            ki          => ki_currentSetpoint,
            kd          => kd_currentSetpoint,
            setpoint    => positional_setpoint,
            reading     => positional_reading_currentSetpoint,
            max_p_pid   => to_sfixed(0.9999, 0, -17),
            max_i_pid   => to_sfixed(0.9999, 0, -17),
            max_d_pid   => to_sfixed(0.9999, 0, -17),
            max_pid_pid => current_setpoint_move,
            min_p_pid   => to_sfixed(-0.9999, 0, -17),
            min_i_pid   => to_sfixed(-0.9999, 0, -17),
            min_d_pid   => to_sfixed(-0.9999, 0, -17),
            min_pid_pid => to_sfixed(0.0, 0, -17),
            pid_out     => current_setpoint
            );

    PID_current : component FOC_fuzzyPID
--        generic map (
        --sampling_time => 0.000000064,  --64ns
--            )
        port map (
    ...
    positional_reading <= vecToSfixed(std_logic_vector(resize(dPosition, 18)), -17);
    positional_reading_currentSetpoint <= vecToSfixed(std_logic_vector(resize(-(abs(dPosition)), 18)), -17);
